...
usage="$FUNCNAME <input.fea> <model.rda>
"
...
echo '
	...
	write.table(file=stdout(), data.frame(OUT="OUT",pred),sep="\t",quote=F,row.names=F,col.names=F);
' > $tmp 
cat $1 | R --no-save -f $tmp \
| awk -v OFS="\t" '$1=="OUT"' | cut -f 2-
rm $tmp
}
